...
![](png/sra_link_to_krona.png){width="100%"} The Taxonomy Browser window will then open. This analysis is performed automatically by NCBI SRA for every dataset that is uploaded there. It is a rough metagenomic analysis based on the information stored at NCBI itself. You can see which organisms (including viruses) were found in the sequence data.

![](png/sra_taxonomy_browser.png){width="100%"} The web interface also offers an interactive way to analyse the composition. This tool is called `Krona`{.tool}, which we will launch next.

![](png/sra_link_to_krona.png){width="70%"}

The `Krona`{.tool} tool for interactive analysis will then open. Play around with the interactive display to understand how Krona works!

![](png/krona_demonstration.gif){width="100%"}
:::
...
::: hands-on
<hands-on-title>Your own search</hands-on-title>
...
![](png/sra_own_search.png){width="100%"}

::: comment
<comment-title>Let's discuss your findings!</comment-title>
